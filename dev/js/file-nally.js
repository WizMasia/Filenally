(() => {
    ...
    const SyncPlanner = (() => {
        ...
        const plan = ({ source, target, manifest = {}, trustedManifest = false, direction = 'bidirectional', conflictPolicy = 'latest', contentEquality, stamp = runStamp() }) => {
    ...
    const Controller = (() => {
        ...
        const compare = async () => {
            ...
            try {
                ...
                model.plan = SyncPlanner.plan({ source: model.sourceFiles, target: model.targetFiles, manifest: model.profile.manifest, trustedManifest: model.trustedProfile, direction: elements.direction.value, conflictPolicy: elements.policy.value, contentEquality });
                renderRows();
